...
- Review fix: `StaticContractRuntimeProvider` now snapshots and restores its live `ContractEscapeResolutionRuntime` state when `RuntimeKernelBootstrapper.EventsReconfigured` swaps event hubs, preserving accepted contracts and search-clear payout progress.
...
- Targeted PlayMode regression: `Reloader.Core.Tests.PlayMode.StaticContractRuntimeProviderPlayModeTests` `2/2` passed
