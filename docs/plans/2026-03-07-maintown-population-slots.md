...
> **Plan update (2026-03-08, review follow-up):** Replacement debt may record `spawnAnchorId` for persistence, but the vacated civilian's anchor remains authoritative for the stable slot contract. `CivilianPopulationModule.ValidateModuleState()` should reject debt whose `spawnAnchorId` diverges from the referenced dead civilian, and `CivilianPopulationRuntimeBridge.ExecutePendingReplacements()` should still rebuild from the vacated civilian anchor defensively if malformed in-memory debt slips through after load.

> **Plan update (2026-03-08, review follow-up):** Replacement execution already preserves slot-anchor stability by minting the new civilian from the vacated record's `SpawnAnchorId`, not from the queued debt payload. Keep that behavior locked with focused runtime coverage instead of adding redundant slot-migration logic unless save validation later needs to reject malformed-but-benign debt payloads explicitly.
