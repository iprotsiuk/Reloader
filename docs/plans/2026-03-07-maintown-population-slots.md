...
> **Plan update (2026-03-08, review follow-up):** Authored `MainTown` starter anchors must occupy distinct world positions so starter civilians do not spawn stacked at `(0,0,0)`. Pending replacement debt invariants must also be enforced at the slot level: a pending debt is invalid if its dead civilian's `populationSlotId` already has a live occupant, and multiple pending debts may not target the same stable slot even if they reference different retired civilians.
