    ...
    public class WebSocketTransportClient : IMcpTransportClient, IDisposable
    {
        ...
        private async Task ReceiveLoopAsync(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                {
                    if (IsShutdownInProgress(token) || IsExpectedShutdownException(wse))
                    {
                        break;
                    }
                ...
                {
                    if (IsShutdownInProgress(token) || IsExpectedShutdownException(ex))
                    {
                        break;
                    }
        ...
        private bool IsShutdownInProgress(CancellationToken token)
        {
            return token.IsCancellationRequested || _lifecycleCts?.IsCancellationRequested == true;
        }
        ...
        private static bool IsExpectedShutdownException(Exception exception)
        {
            return exception is OperationCanceledException
                ...
                || (exception is InvalidOperationException invalidOperationException
                    && IsExpectedShutdownMessage(invalidOperationException.Message));
        }

        private static bool IsExpectedShutdownMessage(string message)
        {
            ...
            return message.IndexOf("WebSocket is not initialised", StringComparison.OrdinalIgnoreCase) >= 0
                || message.IndexOf("WebSocket is not open", StringComparison.OrdinalIgnoreCase) >= 0;
        }
