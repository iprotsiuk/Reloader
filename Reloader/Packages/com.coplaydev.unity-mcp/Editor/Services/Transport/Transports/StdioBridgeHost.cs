...
namespace MCPForUnity.Editor.Services.Transport.Transports
{
    class QueuedCommand
    {
        ...
        public CancellationTokenSource ExecutionCts;
        public bool ResponseAbandoned;
    }
    ...
    public static class StdioBridgeHost
    {
        ...
        private static void DrainQueuedCommands(string reason)
        {
            ...
            lock (lockObj)
            {
                ...
                pending = new List<QueuedCommand>(commandQueue.Values);
                commandQueue.Clear();
            }
            ...
            foreach (var queued in pending)
            {
                ...
                queued.ResponseAbandoned = true;
                try { queued.ExecutionCts?.Cancel(); } catch { }
                try { queued.Tcs?.TrySetCanceled(); } catch { }
                try { queued.ExecutionCts?.Dispose(); } catch { }
            }

            if (IsDebugEnabled())
            {
                McpLog.Info($"Cleared {pending.Count} queued command(s): {reason}", always: false);
            }
        }
        ...
        public static void Stop()
        {
            ...
            DrainQueuedCommands("bridge_stop");
        ...
        private static async Task HandleClientAsync(TcpClient client, CancellationToken token)
        {
            ...
            using (NetworkStream stream = client.GetStream())
            {
                ...
                try
                {
                    ...
                    while (isRunning && !token.IsCancellationRequested)
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    AbandonQueuedCommand(commandId, "stdio_wait_timeout");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                AbandonQueuedCommand(inFlightCommandId, "client_loop_error");
                                inFlightCommandId = null;
                            }
                ...
                finally
                {
                    ...
                    if (remaining == 0)
                    {
                        DrainQueuedCommands("last_client_disconnected");
                    }
        ...
        private static void ProcessCommands()
        {
            ...
            try
            {
                ...
                if (!HasActiveClients())
                {
                    DrainQueuedCommands("no_active_clients");
                    return;
                }
                ...
                lock (lockObj)
                {
                    ...
                    if (staleIds != null)
                    {
                        foreach (var sid in staleIds)
                        {
                            ...
                            try { staleCmd.ExecutionCts?.Cancel(); } catch { }
                            try { staleCmd.ExecutionCts?.Dispose(); } catch { }
                        }
                ...
                foreach (var item in work)
                {
                    ...
                    if (queuedCommand.ResponseAbandoned || (queuedCommand.ExecutionCts?.IsCancellationRequested ?? false))
                    {
                        lock (lockObj) { commandQueue.Remove(id); }
                        try { queuedCommand.ExecutionCts?.Dispose(); } catch { }
                        continue;
                    }
        ...
        private static void ExecuteQueuedCommand(string commandId, QueuedCommand queuedCommand)
        {
            async void Runner()
            {
                string payload = queuedCommand.CommandJson;
                TaskCompletionSource<string> completionSource = queuedCommand.Tcs;
                CancellationTokenSource executionCts = queuedCommand.ExecutionCts;
                CancellationTokenSource linkedCts = null;
                try
                {
                    if (queuedCommand.ResponseAbandoned || (executionCts?.IsCancellationRequested ?? false))
                    {
                        completionSource.TrySetCanceled();
                        return;
                    }

                    using var timeoutCts = new CancellationTokenSource(FrameIOTimeoutMs);
                    CancellationToken commandToken = timeoutCts.Token;
                    if (executionCts != null)
                    {
                        linkedCts = CancellationTokenSource.CreateLinkedTokenSource(timeoutCts.Token, executionCts.Token);
                        commandToken = linkedCts.Token;
                    }

                    string response = await TransportCommandDispatcher.ExecuteCommandJsonAsync(payload, commandToken).ConfigureAwait(true);
                    if (!queuedCommand.ResponseAbandoned)
                    {
                ...
                {
                    if (queuedCommand.ResponseAbandoned || completionSource.Task.IsCompleted)
                    {
                        return;
                    }
                ...
                finally
                {
                    ...
                    try { executionCts?.Dispose(); } catch { }
                }
            }
        ...
        private static void AbandonQueuedCommand(string commandId, string reason)
        {
            ...
            lock (lockObj)
            {
                if (!commandQueue.TryGetValue(commandId, out queued))
                {
                    return;
                }
            ...
            if (IsDebugEnabled())
            {
                McpLog.Info($"Abandoned queued command {commandId} ({reason})", always: false);
            }
        }
