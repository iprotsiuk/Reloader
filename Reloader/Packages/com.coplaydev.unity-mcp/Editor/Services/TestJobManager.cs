...
namespace MCPForUnity.Editor.Services
{
    ...
    internal static class TestJobManager
    {
        ...
        /// </summary>
        /// <returns>True if any stuck/orphaned state was cleared.</returns>
        public static bool ClearStuckJob()
        {
            bool clearedAnyState = false;
            ...
            lock (LockObj)
            {
                if (!string.IsNullOrEmpty(_currentJobId))
                {
                    ...
                    _currentJobId = null;
                    clearedAnyState = true;
                }
            }

            // Recover ghost state: running status can outlive job state if PlayMode init/finalization fails.
            if (TestRunStatus.IsRunning && string.IsNullOrEmpty(CurrentJobId))
            {
                TestRunStatus.MarkFinished();
                clearedAnyState = true;

                if (!string.IsNullOrEmpty(clearedJobId))
                {
                    McpLog.Warn($"[TestJobManager] Cleared stuck job {clearedJobId} and reset run status.");
                }
                else
                {
                    McpLog.Warn("[TestJobManager] Cleared orphaned run status with no current job id.");
                }
            }
        ...
        internal static TestJob GetJob(string jobId)
        {
            ...
            if (shouldClearRunStatus && TestRunStatus.IsRunning)
            {
                TestRunStatus.MarkFinished();
                McpLog.Warn($"[TestJobManager] Cleared run status after initialization timeout for job {jobId}.");
            }
        ...
        private static void FinalizeFromTask(string jobId, Task<TestRunResult> task)
        {
            ...
            lock (LockObj)
            {
                ...
                else if (existing.Status != TestJobStatus.Running)
                {
                    // RunFinished probably finalized first; only clean stale pointer if needed.
                    if (_currentJobId == jobId)
                    {
                        _currentJobId = null;
                        shouldClearRunStatus = true;
                ...
                else
                {
                    ...
                    // FinalizeFromTask is the fallback when RunFinished did not complete cleanup.
                    shouldClearRunStatus = true;
                    shouldPersist = true;
                }
            }
            ...
            {
                TestRunStatus.MarkFinished();
                McpLog.Warn($"[TestJobManager] Cleared run status via task finalizer for job {jobId}.");
            }
