...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class GameObjectSerializer
    {
        ...
        private static void AddSerializableValue(Dictionary<string, object> dict, string name, Type type, object value)
        {
            ...
            // Skip TransformHandle values (and wrappers containing them). These can become invalid/destroyed
            // at runtime and throw during reflection-based serialization.
            if (ContainsTransformHandleType(type) || ContainsTransformHandleType(value.GetType()))
            {
                return;
            }

            // Unity's destroyed-object "fake null" must be handled explicitly when boxed as object.
        ...
        private static bool ContainsTransformHandleType(Type type)
        {
            if (type == null)
            {
                return false;
            ...
            if (type.IsArray)
            {
                return ContainsTransformHandleType(type.GetElementType());
            }
            ...
            {
                if (ContainsTransformHandleType(genericArg))
                {
                    return true;
                }
            }
        ...
        private static bool IsTransformHandleType(Type type)
        {
            string fullName = type.FullName;
            return !string.IsNullOrEmpty(fullName) &&
                   fullName.EndsWith("TransformHandle", StringComparison.Ordinal);
        }
