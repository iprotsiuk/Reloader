    ...
    public static class CommandRegistry
    {
        ...
        private static void CompleteAsyncCommand(string commandName, TaskCompletionSource<string> tcs, object result)
        {
            try
            {
                ...
                {
                    LogAsyncTcsAlreadyCompleted(commandName, tcs, "success");
                }
            }
            catch (Exception ex)
            {
        ...
        private static void ReportAsyncFailure(string commandName, TaskCompletionSource<string> tcs, Exception ex)
        {
            ...
            if (!tcs.TrySetResult(json))
            {
                LogAsyncTcsAlreadyCompleted(commandName, tcs, "error");
            }
        }

        private static void LogAsyncTcsAlreadyCompleted(string commandName, TaskCompletionSource<string> tcs, string completionKind)
        {
            if (tcs?.Task?.IsCanceled == true)
            {
                McpLog.Info($"Ignoring late async {completionKind} completion for canceled command '{commandName}'", always: false);
                return;
            }

            McpLog.Warn($"TCS for async command '{commandName}' was already completed ({completionKind})");
        }
    }
}
